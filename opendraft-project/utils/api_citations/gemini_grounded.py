...
class GeminiGroundedClient(BaseAPIClient):
    ...
    def __init__(
        ...
        # Read Gemini endpoint from env (preferred), fallback to Google official endpoint.
        raw_endpoint = str(os.getenv('GEMINI_API_ENDPOINT', '') or '').strip()
        if raw_endpoint:
            # Accept:
            # - api.linkapi.org
            # - https://api.linkapi.org
            # - https://api.linkapi.org/v1beta/models
            if raw_endpoint.startswith(('http://', 'https://')):
                parsed = urlparse(raw_endpoint)
                if parsed.path and parsed.path not in ('', '/'):
                    proxy_endpoint = raw_endpoint.rstrip('/')
                else:
                    proxy_endpoint = raw_endpoint.rstrip('/') + '/v1beta/models'
            else:
                proxy_endpoint = f"https://{raw_endpoint.rstrip('/')}/v1beta/models"
        else:
            proxy_endpoint = 'https://generativelanguage.googleapis.com/v1beta/models'
        super().__init__(
