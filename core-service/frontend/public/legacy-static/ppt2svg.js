...
function Ppt2Svg(_svg, svgWidth, svgHeight) {
    ...
    function toPaint(paint, anchor, params) {
        if (!paint) {
            ...
            let colors = Array.isArray(gradient.colors) ? gradient.colors : []
            if (colors.length === 0) {
                return 'transparent'
            }
            let x = 0, y = 0
            let width = Number(anchor[2]), height = Number(anchor[3])
            ...
            if (gradient.gradientType == 'circular') {
                // 射线
                let insets = Array.isArray(gradient.insets) ? gradient.insets : [0, 0, 0, 0]
                let topInset = Number(insets[0]) || 0
                let rightInset = Number(insets[1]) || 0
                let bottomInset = Number(insets[2]) || 0
                let leftInset = Number(insets[3]) || 0
                let cx = centerX + width * (rightInset - leftInset) / 2
                let cy = centerY + height * (topInset - bottomInset) / 2
                ...
                let _r = (Math.max(0, rightInset) * 100).toFixed(0) + '%'
                ...
                let endX = x + width
                let endY = centerY
                let angle = Number(gradient.angle)
            ...
            let fractions = Array.isArray(gradient.fractions) ? gradient.fractions : []
            for (let i = 0; i < colors.length; i++) {
                let color = toColor(colors[i])
                let fraction = Number(fractions[i])
                if (!Number.isFinite(fraction)) {
                    fraction = colors.length === 1 ? 0 : (i / (colors.length - 1))
                }
                fraction = Math.max(0, Math.min(1, fraction))
                let offset = (fraction * 100).toFixed(0) + '%'
