  ...
  async callTranslationAPI(systemPrompt, userPrompt, model, apiKey, options = {}) {
    ...
    if (model === 'custom') {
      ...
      const predefinedConfigs = {
        ...
        'lingine_en': {
          endpoint: '/api/literature/translate',
          modelName: 'LINGINE EN (deepl-en)',
          headers: { 'Content-Type': 'application/json' },
          bodyBuilder: (sys, user, ctx = {}) => ({
            text: user,
            systemPrompt: sys || undefined,
            targetLang: (ctx && ctx.targetLang) ? ctx.targetLang : 'zh'
          }),
    ...
    const requestBody = apiConfig.bodyBuilder
      ? apiConfig.bodyBuilder(systemPrompt, userPrompt, {
          targetLang: (options && options.targetLang) ? options.targetLang : 'zh',
          processedText: userPrompt
        })
      : {
