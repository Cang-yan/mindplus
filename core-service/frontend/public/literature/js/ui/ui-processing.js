...
(function(global) {
    ...
    function updateProcessButtonState(pdfFiles, isProcessing) {
        ...
        if (selectedTranslationModelName !== 'none') {
            const keyRequiredFn = typeof global.isTranslationModelUserKeyRequired === 'function'
                ? global.isTranslationModelUserKeyRequired
                : (modelName => !!modelName && modelName !== 'none' && modelName !== 'lingine_en');

            // LINGINE EN 由后端统一处理，无需用户配置 API Key
            if (selectedTranslationModelName === 'lingine_en') {
                translationKeysAvailable = true;
                translationModelDisplayName = 'LINGINE EN';
            ...
            // 使用 KeyProvider 检查
            if (currentTranslationModelForProvider && keyRequiredFn(currentTranslationModelForProvider)) {
    ...
    function updateValidationAlert(effectiveFiles, hasPdfFiles, ocrConfigAvailable, translationKeysAvailable, selectedTranslationModel, translationModelDisplayName, isProcessing, ocrEngine, ocrConfigValid, ocrConfigMessage) {
        ...
        if (hasPdfFiles && !ocrConfigAvailable) {
            if (ocrEngine === 'none') {
                issues.push({
                    ...
                    message: '您上传了 PDF 文件，但当前选择了"不需要OCR"。请在上方 OCR 设置中选择"本地解析（仅文本PDF）"或"LINGINE OCR"。',
                    icon: 'carbon:warning-alt',
                    actions: []
                });
            } else if (!ocrConfigValid) {
                const engineNames = { mistral: 'Mistral OCR', mineru: 'MinerU', doc2x: 'Doc2X', lingine: 'LINGINE OCR' };
        ...
        if (selectedTranslationModel && selectedTranslationModel !== 'none' && !translationKeysAvailable) {
            ...
            const message = isCustomSource
                ...
                : (isBackendMode
                    ? `模型 "${translationModelDisplayName}" 在后端模式下没有可用的 API Key。请在服务端完成配置后再处理。`
                    : `模型 "${translationModelDisplayName}" 没有可用的 API Key。请添加 Key 后再处理，配置后可点击右上角刷新按钮更新状态。`);

            const keyAction = isBackendMode
                ? { text: '刷新状态', onClick: () => {
                    if (typeof window.refreshValidationState === 'function') {
                        window.refreshValidationState();
                    }
