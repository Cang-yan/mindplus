...
class OcrManager {
  ...
  getConfig() {
    ...
    switch (engine) {
      ...
      case 'lingine':
        // LINGINE OCR 由后端处理，无需用户侧配置
        return { engine: 'lingine' };
  ...
  createAdapter(config) {
    switch (config.engine) {
      ...
      case 'lingine':
        if (typeof LingineOcrAdapter === 'undefined') {
          throw new Error('LingineOcrAdapter not loaded');
        }
        return new LingineOcrAdapter(config);
  ...
  validateConfig() {
    ...
    switch (config.engine) {
      case 'local':
      case 'lingine':
        // 本地解析/LINGINE OCR 不需要用户侧配置，总是有效
