...
const fileType = fileToProcess.name.split('.').pop().toLowerCase();
    ...
    try {
        ...
        if (selectedTranslationModelName !== 'none') {
            const isKeylessTranslationModel = selectedTranslationModelName === 'lingine_en';
            const translationKeyValue = (translationKeyObject && translationKeyObject.value) ? translationKeyObject.value : '';
            if (!translationKeyValue && !isKeylessTranslationModel) {
                ...
                if (typeof addProgressLog === "function") {
                    if (isKeylessTranslationModel) {
                        addProgressLog(`${logPrefix} 开始翻译 (${selectedTranslationModelName}, 后端代理模式)`);
                    } else {
                ...
                if (!shouldUseStructuredTranslation) {
                    ...
                    if (typeof addProgressLog === "function") addProgressLog(`${logPrefix} 翻译完成`);
                        ...
                        if (
                            !isKeylessTranslationModel &&
                            translationKeyObject &&
                            translationKeyObject.id &&
                            error.message &&
                            (
                                error.message.includes('无效') ||
                                error.message.includes('未授权') ||
