...
/**
 * 判断翻译模型是否需要用户在前端配置 API Key。
 * 说明：
 * - lingine_en 走本地后端代理，不需要用户侧 Key。
 * - none 表示不翻译，不需要 Key。
 * @param {string} modelName
 * @returns {boolean}
 */
function isTranslationModelUserKeyRequired(modelName) {
...
function isBackendModeEnabled() {
    return !!(typeof window !== 'undefined' && window.storageAdapter && window.storageAdapter.isFrontendMode === false);
}

/**
 * 统一生成翻译模型缺少 Key 的提示文案。
 * @param {string} modelDisplayName
 * @param {boolean} isCustomSource
 * @returns {string}
 */
function buildMissingTranslationKeyMessage(modelDisplayName, isCustomSource) {
    if (isCustomSource) {
        return `源站 "${modelDisplayName}" 没有可用的 API Key。请点击源站信息下方的"管理该站点 API Key"按钮添加Key。`;
    }

    if (isBackendModeEnabled()) {
        return `模型 "${modelDisplayName}" 在后端模式下没有可用的 API Key。请在后端服务端配置该模型 Key 后重试。`;
    }

    return `模型 "${modelDisplayName}" 没有可用的 API Key。请点击页面右上方的"模型与Key管理"按钮添加Key。`;
}

if (typeof window !== 'undefined') {
    window.isTranslationModelUserKeyRequired = isTranslationModelUserKeyRequired;
    window.isBackendModeEnabled = isBackendModeEnabled;
    window.buildMissingTranslationKeyMessage = buildMissingTranslationKeyMessage;
}
...
async function handleProcessClick() {
    ...
    if (selectedTranslationModelName !== 'none') {
        if (selectedTranslationModelName === 'lingine_en') {
            // LINGINE EN 通过后端代理调用，无需用户配置 API Key
            ...
            if (!translationModelConfigForProcess) translationModelConfigForProcess = {};
            if (!translationModelConfigForProcess.displayName) {
                translationModelConfigForProcess.displayName = 'LINGINE EN';
            }
            addProgressLog('使用预设翻译模型: lingine_en（后端代理，无需前端 Key）');
        } else if (selectedTranslationModelName === 'custom') {
        ...
        if (currentTranslationModelForProvider && isTranslationModelUserKeyRequired(currentTranslationModelForProvider)) {
            ...
            if (!translationKeyProvider.hasAvailableKeys()) {
                ...
                let errorMsg = buildMissingTranslationKeyMessage(modelDisplayName, isCustomSource);
    ...
    const processQueue = async () => {
        while (pendingIndices.size > 0 || activeProcessingCount > 0) {
            while (pendingIndices.size > 0 && activeProcessingCount < concurrencyLevel && !stopRequestedByOcr) {
                    ...
                    .then(async result => {
                        if (result && result.keyInvalid) {
                            ...
                            if (nextRetryCount <= MAX_RETRIES) {
                                ...
                                const isOcrFailure = /ocr|pdf|mistral|mineru|doc2x|lingine/i.test(String(errorMsg || ''));
                                if (isOcrFailure && !stopRequestedByOcr) {
                                    stopRequestedByOcr = true;
                                    if (pendingIndices.size > 0) {
                                        for (const pendingIndex of pendingIndices) {
                                            const pendingFile = filesToProcess[pendingIndex];
                                            allResults[pendingIndex] = {
                                                file: pendingFile,
                                                error: '已因 OCR 连续失败达到重试上限而终止批处理'
                                            };
                                            errorCount++;
                                        }
                                        pendingIndices.clear();
                                    }
                                    addProgressLog(`[${currentFile.name}] OCR 连续失败达到上限，已终止后续任务。`);
                                }
                            }
                        }
                    })
    ...
    try {
        ...
        if (stopRequestedByOcr) {
            addProgressLog('批处理已因 OCR 连续失败达到重试上限而提前结束。');
        }
