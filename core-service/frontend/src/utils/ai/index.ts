import { getAISettings, getProviderApiKey, getProviderBaseUrl } from './config'
...
export function streamGenerate(
  ...
  ;(async () => {
    try {
      ...
      if (hasKey) {
        const baseUrl = getProviderBaseUrl(finalProvider, settings)
        if (!baseUrl) throw new Error(`Base URL is not configured for provider: ${finalProvider}`)
